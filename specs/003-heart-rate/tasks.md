...
- [X] T004 [P] Create `HeartRateReading` (bpm, confidence, source, measuredAtEpochMillis, sequence) in `core/model/src/main/kotlin/io/github/andrewkomkov/greenpods/core/model/HeartRateReading.kt` — this task said `HeartRate.kt`, which is not the file that was written, and the path is corrected here rather than left to send the next reader looking for it — replacing `HeartRateSample`; reject BPM outside 25..250 at construction; `measuredAtEpochMillis` is always a resolved wall-clock time, never the raw report counter — FR-005, FR-009, AD-10
...
- [X] T070 Walk [quickstart.md](./quickstart.md) **§1-§3** on the Pixel 8 with AirPods Pro 3 — pass, 2026-08-04. Off-device suite green; nothing starts unasked (zero tx frames in 60 s); and with the channel held open across a reconnection the accessory announces its services, `0x13` is discovered from that announcement rather than assumed, reports arrive at 1 Hz, and a heart rate reaches the screen — confirmed on the device by the project owner. **Scope narrowed to §1-§3 on 2026-08-08**: this task read "§1-§7" while its own note said §4-§7 were not walked, so the tick claimed more than was done. The outstanding half is T070a, and it is unticked
- [ ] T070a Walk [quickstart.md](./quickstart.md) **§4-§7** on the Pixel 8 with AirPods Pro 3 — not run. The hardware became unavailable on 2026-08-04 and the two defects found in §3 (T085-T087) had consumed the session. Stop-frame correctness, report-body exclusion from the log and the discovery path are each verified in the earlier runs recorded in `docs/protocol-research.md`, which is why this was mistaken for done — but an earlier run of a different build is not this walk. **Open, and blocked on the accessory** — SC-001, SC-007, SC-008 `scripts/hardware-walk.sh` drives these sections along with the other hardware-blocked walks.
