...
- [ ] T082 **Capture the request that makes the accessory announce its HID services.** On a freshly opened channel AirPods Pro 3 sends no `0x17` frames at all, so `HidDescriptorParser` never runs, no service id is discovered, and the controller waits in `STARTING` for ever. The earlier capture only showed descriptors because another AAP client had already prompted the accessory. Writing the start frame directly with the known id produces reports immediately, so the sensor is willing and only discovery is missing. Until this frame is known the AAP route depends on another app having run first, which is not shippable — FR-002, R-1, and `docs/protocol-research.md` "The descriptors are not sent unprompted"
