...
- [ ] T038 Walk the whole of [quickstart.md](./quickstart.md) on a device and record which steps were actually executed — a green suite is not a substitute, and two instrumented tests in this project already pass only when no AirPods are nearby. **Unticked 2026-08-08: the record this task asks for does not exist.** T034, T035 and T036 each carry a dated note for the section they cover — promotion confirmed, rapid wear changes, notifications denied — and `docs/protocol-research.md` holds the promotion finding. Nothing anywhere says the whole quickstart was walked or which steps were run, and the deliverable here is that record. `specs/004-head-tracking-calibration/quickstart.md` has a "What was actually run" section that is the shape this wants. **Open, and blocked on the accessory** `scripts/hardware-walk.sh` drives this walk along with the other hardware-blocked ones.
