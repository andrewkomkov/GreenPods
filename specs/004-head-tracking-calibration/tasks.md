...
- [X] T009 (FR-020) Change `HeadPoseMapper` in `core/bluetooth/src/main/kotlin/io/github/andrewkomkov/greenpods/core/bluetooth/head/HeadGestureDetector.kt` from an `object` with a `private const SCALE` into a value type constructed from a `HeadCalibration`, with an explicitly-named uncalibrated variant carrying today's constant and today's "this is an approximation" wording. **No mutable global**; the one production call site is `HeadTrackingController.kt:160`
...
- [X] T018 [US1] Make both overridable from adb via `set`, following `hrConfidenceThreshold` — a provisional number is a setting precisely so that calibrating it stays a measurement rather than a rebuild. **Was ticked without the code existing**, and found by the T059 audit: `PlateauDetector.Config` was parameterised but nothing ever constructed it with anything but defaults, so the two numbers were as unreachable as constants. Now `calibrationToleranceUnits` and `calibrationHoldMillis` are settings, persisted, clamped, in `dump`, in `set`, printed by `cal start`, and reaching the detector, the solver's minimum response and the poses' countdown through one `CalibrationSession.from`
...
- [X] T035 [US1] (FR-003) Create `HeadCalibrationScreen` in `feature/settings/src/main/kotlin/io/github/andrewkomkov/greenpods/feature/settings/HeadCalibrationScreen.kt` — renders the session and owns no decisions. Countdown per pose (FR-005), using `GreenPodsMotion` tokens rather than raw `tween`/`spring`
...
- [X] T038 [US1] (FR-001) Add the entry point to `feature/settings/src/main/kotlin/io/github/andrewkomkov/greenpods/feature/settings/HeadGestureScreen.kt`, shown **locked with its reason** when the stream cannot start (FR-002), reusing `LockedCard` — the same affordance that screen already uses for the same refusal
...
- [X] T052 [US3] (SC-004) Report a pose with no held segment as `NotHeld`, name it, and offer to repeat it (FR-015)
...
- [ ] T060 (SC-002) Walk section 7 on hardware: AirPods Pro 3, AAP channel open, a real run of the wizard. Record what the response matrix says. **If it reports cross-coupling, that is the result** — write it down and stop, rather than adjusting thresholds until numbers appear. **Open, and blocked on the accessory**: every sample this feature has seen was injected, so it has measured its own machinery and nothing else **Attempted 2026-08-09 on the Pixel 8 with AirPods Pro 3, channel open, and left open.** The session reached the neutral hold and spent itself on two defects that only hardware could show: the 900-unit tolerance sat below the drift of a still head, and the pose countdown was the same number as the plateau it had to contain, so a perfectly still pose was refused for being seven milliseconds short. Both are fixed and written up in `docs/protocol-research.md`. No yaw, pitch or roll pose was completed, so the response matrix - the thing this task is for - is still unmeasured and the cross-coupling claim is still unpinned **Run 2026-08-09 and it produced the matrix**: yaw moved o2 by +14398 and o3 by -8673, pitch moved o3 by +4301, roll moved o2 by -8076, and o1 moved less than the tolerance for all three. Reported as INCONCLUSIVE, MISMATCHED and MISMATCHED - not CROSS_COUPLED, so the fields do separate and the per-axis model survives; it is the O1/O2/O3 assignment that is wrong. A raw capture taken in the same session then found a gravity vector at report offsets 44/46/48, which changes what this feature should do - see `docs/protocol-research.md`. Left open deliberately: the run stored nothing, and the finding needs a spec amendment rather than a tick
...
- [ ] T064 Time a full run end to end with a person on the screen, reading the instructions rather than knowing them — **SC-001** asks for under three minutes and no task ever checked it. Found by `/speckit-analyze` on 2026-08-09, after a manual audit of these ticks had missed it: every other requirement is cited by some task, and this one was cited by none. Time it during T060, since it needs the same person and the same live wizard; if it runs long, the finding is the number, not a reason to shorten the holds — the hold duration is a measurement parameter now (T018), not a pacing choice. **Open, and blocked on the accessory**
